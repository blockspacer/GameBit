...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new GameBit address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"GameBitaddress\"    (string) The new GameBit address\n"
            "\nExamples:\n"
	...
	// GameBit to send v1 address by default
    return CGameBitAddress(keyID, CChainParams::ADDRESS_OLDSYS).ToString();
}
// GameBit
UniValue getv2address(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
			...
			"\nReturns a new GameBit (starts with 1) address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
    ...
    return CGameBitAddress(keyID).ToString();
}
UniValue getzaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"address\"        (string) GameBit alias or address to convert to ZCash address.\n"
            "\nResult:\n"
    ...
    CGameBitAddress sysAddress(strAddress);
	...
	if(!sysAddress.vchRedeemScript.empty())
	{
		...
		sysAddress = CGameBitAddress(innerID, CChainParams::ADDRESS_ZEC);
		return sysAddress.ToString();
	}
	else
	{
		CPubKey pubkey(sysAddress.vchPubKey);
		return CGameBitAddress(pubkey.GetID(), CChainParams::ADDRESS_ZEC).ToString();
	}
}
CGameBitAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    CPubKey pubKey;
    ...
    return CGameBitAddress(pubKey.GetID());
}
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current GameBit address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"GameBitaddress\"   (string) The account GameBit address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new GameBit address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"GameBitaddress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"GameBitaddress\"  (string, required) The GameBit address to be associated with an account.\n"
    ...
    CGameBitAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid GameBit address");
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"GameBitaddress\"\n"
            "\nDEPRECATED. Returns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"GameBitaddress\"  (string, required) The GameBit address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"GameBitaddress\"  (string) a GameBit address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    BOOST_FOREACH(const PAIRTYPE(CGameBitAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        const CGameBitAddress& address = item.first;
...
// GameBit: Send service transactions
void SendMoneyGameBit(const vector<CRecipient> &vecSend, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew, const CWalletTx* wtxAliasIn=NULL, int nTxOutAlias = 0, bool GameBitMultiSigTx=false, const CCoinControl* coinControl=NULL, const CWalletTx* wtxLinkAliasIn=NULL, int nTxOutLinkAlias = 0)
{
    ...
    std::string strError;
    int nChangePosRet = -1;
    if (!pwalletMain->CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet, strError, coinControl, !GameBitMultiSigTx, wtxAliasIn, nTxOutAlias, true, wtxLinkAliasIn, nTxOutLinkAlias)) {
    ...
    if (!GameBitMultiSigTx && !pwalletMain->CommitTransaction(wtxNew, reservekey))
        throw runtime_error("GameBit_RPC_ERROR ERRCODE: 9000 - " + _("The GameBit alias you are trying to use for this transaction is invalid or has been updated and not confirmed yet! Please wait a block and try again..."));
}
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress \"GameBitaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
            "\nSend an amount to a given address.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"GameBitaddress\"  (string, required) The GameBit address to send to.\n"
            ...
            "                             The recipient will receive less GameBits than you enter in the amount field.\n"
            "\nResult:\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"GameBitaddress\",     (string) The GameBit address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(CGameBitAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(CGameBitAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CGameBitAddress(address).Get())->second.name);
            }
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"GameBitaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"GameBitaddress\"  (string, required) The GameBit address to use for the private key.\n"
    ...
    CGameBitAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"GameBitaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given GameBitaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"GameBitaddress\"  (string, required) The GameBit address for transactions.\n"
    ...
    // GameBit address
    CGameBitAddress address = CGameBitAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid GameBit address");
	// GameBit
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toGameBitaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a GameBit address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toGameBitaddress\"  (string, required) The GameBit address to send funds to.\n"
    ...
    string strAccount = AccountFromValue(params[0]);
    CGameBitAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid GameBit address");
    CAmount nAmount = AmountFromValue(params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The GameBit address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less GameBits than you enter in their corresponding amount field.\n"
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a GameBit address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of GameBit addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) GameBit address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"GameBitaddress\"  (string) A GameBit address associated with the keys.\n"
    ...
    return CGameBitAddress(innerID).ToString();
}
...
UniValue addwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    return CGameBitAddress(w.result).ToString();
}
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    map<CGameBitAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CGameBitAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CGameBitAddress, tallyitem>::iterator it = mapTally.find(address);
        ...
        {
			// GameBit v1 addy by default
			CTxDestination dest = address.Get();
			CGameBitAddress v1addr;
...
{
	// GameBit address is v1 address by default for backward compatibility, v2 address is new scheme
    CGameBitAddress addr;
...
// GameBit
string GetGameBitTransactionDescription(const int op, const vector<vector<unsigned char> > &vvchArgs, const string &type, const CWalletTx& wtx, string& responseEnglish, string& responseGUID, string& responseGUID1)
{
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const COutputEntry& s, listSent)
        {
			...
			// GameBit
			if(wtx.nVersion == GetGameBitTxVersion() && (IsGameBitScript(wtx.vout[s.vout].scriptPubKey, op, vvchArgs) || (wtx.vout[s.vout].scriptPubKey[0] == OP_RETURN && DecodeAndParseGameBitTx(wtx, op, nOut, vvchArgs))))
			{
				...
				strResponse = GetGameBitTransactionDescription(op, vvchArgs, "send", wtx, strResponseEnglish, strResponseGUID, strResponseGUID1);
        ...
        BOOST_FOREACH(const COutputEntry& r, listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                if (fLong)
                    WalletTxToJSON(wtx, entry);
				// GameBit
				if(wtx.nVersion == GetGameBitTxVersion() && (IsGameBitScript(wtx.vout[r.vout].scriptPubKey, op, vvchArgs) || (wtx.vout[r.vout].scriptPubKey[0] == OP_RETURN && DecodeAndParseGameBitTx(wtx, op, nOut, vvchArgs))))
				{
					if (mapSysTx[wtx.GetHash()])
						continue;
					mapSysTx[wtx.GetHash()] = true;
					...
					string strResponseGUID = "";
					string strResponseGUID1 = "";
					strResponse = GetGameBitTransactionDescription(op, vvchArgs, "recv", wtx, strResponseEnglish, strResponseGUID, strResponseGUID1);
					entry.push_back(Pair("systx", strResponse));
					entry.push_back(Pair("systype", strResponseEnglish));
					entry.push_back(Pair("sysguid", strResponseGUID));
					if(!strResponseGUID1.empty())
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"GameBitaddress\",    (string) The GameBit address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"GameBitaddress\",    (string) The GameBit address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"GameBitaddress\",   (string) The GameBit address involved in the transaction\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending GameBits\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending GameBit\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"GameBitaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; GameBit server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending GameBits.\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of GameBit addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) GameBit address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the GameBit address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CGameBitAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid GameBit address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        if (fValidAddress) {
			...
			entry.push_back(Pair("v2address", CGameBitAddress(address).ToString()));
...
UniValue fundrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"     (string, optional, default pool address) The GameBit address to receive the change\n"
    ...
    if (params.size() > 1) {
      if (params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            CGameBitAddress address(options["changeAddress"].get_str());

            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid GameBit address");
...
extern UniValue GameBitsignrawtransaction(const UniValue& params, bool fHelp);
extern UniValue GameBitdecoderawtransaction(const UniValue& params, bool fHelp);
...
{ //  category              name                        actor (function)           okSafeMode
	...
	{ "wallet", "GameBitsignrawtransaction",		 &GameBitsignrawtransaction,	false },
	{ "wallet", "GameBitdecoderawtransaction",		 &GameBitdecoderawtransaction,	false },
