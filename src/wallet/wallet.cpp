...
extern bool IsGameBitScript(const CScript& scriptPubKey, int &op, vector<vector<unsigned char> > &vvchArgs);
extern int GetGameBitTxVersion();
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CGameBitAddress(CScriptID(redeemScript)).ToString();
                                 ...
                                 set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, bool bAliasPay) const
{
    ...
    BOOST_FOREACH(const COutput &output, vCoins)
    {
		...
		// GameBit txs are unspendable unless input to another GameBit tx (passed into createtransaction)
		if(pcoin->nVersion == GetGameBitTxVersion())
		{
			int op;
			vector<vector<unsigned char> > vvchArgs;
			if (pcoin->vout.size() >= i && IsGameBitScript(pcoin->vout[i].scriptPubKey, op, vvchArgs) && op != OP_ALIAS_PAYMENT)
				continue;
		}
    ...
    BOOST_FOREACH(const COutPoint& outpoint, vPresetInputs)
    {
        ...
        if (it != mapWallet.end())
        {
			...
			if(pcoin->nVersion == GetGameBitTxVersion())
			{
				int op;
				vector<vector<unsigned char> > vvchArgs;
				if (pcoin->vout.size() >= outpoint.n && IsGameBitScript(pcoin->vout[outpoint.n].scriptPubKey, op, vvchArgs) && op != OP_ALIAS_PAYMENT)
					continue;
			}
...
{
    CAmount nValue = 0;
	// GameBit: get output amount of input transactions for GameBit service calls
	if(wtxInAlias != NULL)
	{
		if (nTxOutAlias < 0)
		{
			strFailReason = _("Can't determine type of alias input into GameBit service transaction");
            return false;
		}
	}
	if(wtxInLinkAlias != NULL)
	{
		...
		{
			strFailReason = _("Can't determine type of linked alias input into GameBit service transaction");
            return false;
		}
	}
	...
	// GameBit: set GameBit tx version if its a GameBit service call
	if(sysTx)
		txNew.nVersion = GetGameBitTxVersion();
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                BOOST_FOREACH (const CRecipient& recipient, vecSend)
                {
					// GameBit pay to alias
					CRecipient myrecipient = recipient;
					CTxDestination payDest;
					int op;
					vector<vector<unsigned char> > vvchArgs;
					if (!myrecipient.scriptPubKey.IsUnspendable() && !IsGameBitScript(myrecipient.scriptPubKey, op, vvchArgs)) {
						if (ExtractDestination(myrecipient.scriptPubKey, payDest)) 
						{
							CGameBitAddress address = CGameBitAddress(payDest);
							address = CGameBitAddress(address.ToString());
							if(address.isAlias)
							{
				...
				// GameBit input credit from input tx
                ...
                CAmount nValueIn = 0;
				// GameBit add input credit to current coin selection
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-GameBit-address
                    ...
                    if (coinControl && !boost::get<CNoDestination>(&coinControl->destChange))
					{
						...
						address = CGameBitAddress(coinControl->destChange);
						address = CGameBitAddress(address.ToString());
						if(!address.vchRedeemScript.empty())
                    ...
                    else
                    {
                       // Note: We use a new key here to keep it from being obvious which side is the change.
						...
						// GameBit pay to input destination as change
						CTxDestination payDest;
						...
						if (ExtractDestination(vecCoins[nLastIndex].first->vout[vecCoins[nLastIndex].second].scriptPubKey, payDest)) 
						{
							...
							address = CGameBitAddress(payDest);
							address = CGameBitAddress(address.ToString());
					...
					// GameBit change as a alias payment				
				...
				// GameBit use max-2 because we want RBF replacement to be on by default on this wallet
                // Fill vin
                //
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CGameBitAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CGameBitAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CGameBitAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CGameBitAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CGameBitAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CGameBitAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CGameBitAddress(dest).ToString(), key);
}
