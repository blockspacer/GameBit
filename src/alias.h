...
const int GameBit_TX_VERSION = 0x7400;
...
bool IsGameBitTxMine(const CTransaction& tx,const std::string &type);
...
bool DecodeAndParseGameBitTx(const CTransaction& tx, int& op, int& nOut, std::vector<std::vector<unsigned char> >& vvch);
...
CAmount convertCurrencyCodeToGameBit(const std::vector<unsigned char> &vchAliasPeg, const std::vector<unsigned char> &vchCurrencyCode, const double &nPrice, const unsigned int &nHeight, int &precision);
...
CAmount convertGameBitToCurrencyCode(const std::vector<unsigned char> &vchAliasPeg, const std::vector<unsigned char> &vchCurrencyCode, const CAmount &nPrice, const unsigned int &nHeight, int &precision);
...
std::string GenerateGameBitGuid();
bool IsAliasOp(int op);
...
bool GetGameBitData(const CTransaction &tx, std::vector<unsigned char> &vchData, std::vector<unsigned char> &vchHash, int& nOut);
bool GetGameBitData(const CScript &scriptPubKey, std::vector<unsigned char> &vchData, std::vector<unsigned char> &vchHash);
...
bool IsGameBitScript(const CScript& scriptPubKey, int &op, std::vector<std::vector<unsigned char> > &vvchArgs);
...
void CleanupGameBitServiceDatabases(int &servicesCleaned);
int aliasunspent(const std::vector<unsigned char> &vchAlias, COutPoint& outpoint);
bool IsMyAlias(const CAliasIndex& alias);
void GetAddress(const CAliasIndex &alias, CGameBitAddress* address, const uint32_t nPaymentOption=1);
void GetAddress(const CAliasIndex &alias, CGameBitAddress* address, CScript& script, const uint32_t nPaymentOption=1);
