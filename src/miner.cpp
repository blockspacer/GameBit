...
extern int GetGameBitDataOutput(const CTransaction& tx);
using namespace std;
...
CBlockTemplate* BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    coinbaseTx.vout[0].scriptPubKey = scriptPubKeyIn;
	 // GameBit Compute final coinbase transaction. Add GameBit dynamic inflation based on service demand
...
void BlockAssembler::AddToBlock(CTxMemPool::txiter iter)
{
	...
	if(iter->GetTx().nVersion == GetGameBitTxVersion())
	{
		nSysBlockTx++;
		if(nSysBlockTx >= 5)
		{
			int nOut = GetGameBitDataOutput(iter->GetTx());
			if (nOut != -1)
