...
{
    CGameBitAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered GameBit address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CGameBitAddress(rcp.address.toStdString()).Get());
...

    {
		...
		// GameBit pass in false to not sign
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal GameBit:URI (GameBit:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CGameBitAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
		...
		if(out.tx->nVersion == GetGameBitTxVersion())
		{
			int op;
			vector<vector<unsigned char> > vvchArgs;
			// any sys tx thats not an alias payment shouldnt show up in listCoins (used by coin control)
			if (out.tx->vout.size() >= out.i && IsGameBitScript(out.tx->vout[out.i].scriptPubKey, op, vvchArgs) && op != OP_ALIAS_PAYMENT)
				continue;
		}
        ...
        if(/*!out.fSpendable || */!ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
		// GameBit
		CGameBitAddress GameBitAddress = CGameBitAddress(address);
		GameBitAddress = CGameBitAddress(GameBitAddress.ToString());
		QString qStrAddress = QString::fromStdString(GameBitAddress.ToString());
		if(GameBitAddress.isAlias)
			qStrAddress = QString::fromStdString(GameBitAddress.aliasName);
...
{
    CTxDestination dest = CGameBitAddress(sAddress).Get();
