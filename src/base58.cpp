// Copyright (c) 2014-2015 The GameBit Core developers
...
// GameBit use aliases as addresses
...
namespace
{
class CGameBitAddressVisitor : public boost::static_visitor<bool>
{
private:
    CGameBitAddress* addr;
	// GameBit support old sys
	CChainParams::AddressType nSysVer;
public:
    CGameBitAddressVisitor(CGameBitAddress* addrIn) : addr(addrIn) {}
	CGameBitAddressVisitor(CGameBitAddress* addrIn, CChainParams::AddressType nSysVer) : nSysVer(nSysVer), addr(addrIn) {}
...
// GameBit aliases as addresses
CGameBitAddress::CGameBitAddress() {
	isAlias = false;
	aliasName = "";
	safeSearch = false;
	safetyLevel = 0;
...
// GameBit support old sys
CGameBitAddress::CGameBitAddress(const CTxDestination &dest, CChainParams::AddressType sysVer) { 
	isAlias = false;
	safeSearch = false;
	safetyLevel = 0;
	aliasName = "";
...
CGameBitAddress::CGameBitAddress(const std::string& strAddress) { 
	isAlias = false;
	aliasName = "";
...
CGameBitAddress::CGameBitAddress(const char* pszAddress) { 
	isAlias = false;
...
// GameBit support old sys
bool CGameBitAddress::Set(const CKeyID& id, CChainParams::AddressType sysVer)
{   
...
bool CGameBitAddress::Set(const CScriptID& id, CChainParams::AddressType sysVer)
{
...
// GameBit support old sys
bool CGameBitAddress::Set(const CTxDestination& dest, CChainParams::AddressType sysVer)
{
    return boost::apply_visitor(CGameBitAddressVisitor(this, sysVer), dest);
}

bool CGameBitAddress::IsValid() const
{
    return IsValid(Params());
}

bool CGameBitAddress::IsValid(const CChainParams& params) const
{
    bool fCorrectSize = vchData.size() == 20;
	// GameBit allow old GameBit address scheme
...
CTxDestination CGameBitAddress::Get() const
{
    if (!IsValid())
...
bool CGameBitAddress::GetKeyID(CKeyID& keyID) const
{
	// GameBit allow old GameBit address scheme
...
bool CGameBitAddress::IsScript() const
{
...
void CGameBitSecret::SetKey(const CKey& vchSecret)
{
...
bool CGameBitSecret::IsValid() const
{
...
bool CGameBitSecret::SetString(const char* pszSecret)
{
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CGameBitSecret::SetString(const std::string& strSecret)
{
