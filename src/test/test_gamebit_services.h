#ifndef GameBit_TEST_TEST_GameBit_SERVICES_H
#define GameBit_TEST_TEST_GameBit_SERVICES_H

#include <stdio.h>
#include <univalue.h>
using namespace std;
/** Testing GameBit services setup that configures a complete environment with 3 nodes.
 */
...
struct GameBitTestingSetup {
    GameBitTestingSetup();
    ~GameBitTestingSetup();
};
struct BasicGameBitTestingSetup {
    BasicGameBitTestingSetup();
    ~BasicGameBitTestingSetup();
};
struct GameBitMainNetSetup {
	GameBitMainNetSetup();
	~GameBitMainNetSetup();
};
#endif
