...
#if defined(NDEBUG)
# error "GameBit cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "GameBit Signed Message:\n";
...
// GameBit
bool CheckGameBitInputs(const CTransaction& tx, const CCoinsViewCache& inputs, bool fJustCheck, int nHeight=0)
{
    ...
    string errorMessage;
    if(tx.nVersion == GetGameBitTxVersion())
    {
        bool good = true;
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
    ...
    // GameBit
    if (fRequireStandard && tx.nVersion != GetGameBitTxVersion() && tx.nVersion >= 2 && VersionBitsTipState(chainparams.GetConsensus(), Consensus::DEPLOYMENT_CSV) != THRESHOLD_ACTIVE) {
...

    {
        ...
        // GameBit
        if (!CheckGameBitInputs(tx, view, true))
            return false;
...
//
// GameBit check header auxpow proof of work
...
// GameBit: block header template's for auxpow integration
template<typename T>
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    std::vector<int> prevheights;
    // GameBit inflate when high demand for services
    ...
    {
        const CTransaction &tx = block.vtx[i];
        // GameBit inflate and regenerate based on opreturn value set when creating the service(user)
        ...
        if(tx.nVersion == GetGameBitTxVersion())
        {
            ...
            if(nSysBlockTx >= 5)
            {
                int nOut = GetGameBitDataOutput(tx);
                if (nOut != -1)
        ...
        if (!tx.IsCoinBase())
        {
            ...
            // GameBit
            if (!CheckGameBitInputs(tx, view, fJustCheck, pindex->nHeight))
                return error("ConnectBlock(): CheckGameBitInputs on %s failed",tx.GetHash().ToString());
            control.Add(vChecks);
        }
    ...
    // GameBit Add dynamic inflation based on service demand
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
            // GameBit, skip auxpow bit 8 or dummy bit 28
            if(bit == 8 || bit == 28)
                continue;
...
{
    // GameBit check auxpow first then fallback to normal check
                     ...
                     ++mi)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (orphanTx.wit.IsNull() && !stateDummy.CorruptionPossible()) {
                            ...
                            // See https://github.com/GameBit/GameBit2/issues/8279 for details.
                            assert(recentRejects);
