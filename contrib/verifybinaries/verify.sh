...
###   This script attempts to download the signature file SHA256SUMS.asc from GameBit.org
...
WORKINGDIR="/tmp/GameBit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://GameBit.org/bin/"
VERSIONPREFIX="GameBit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'GameBit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the GameBit Core binary release signing key installed?"
   fi
