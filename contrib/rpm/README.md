...
The RPM spec file provided here is for GameBit-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://GameBit.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[GameBit git repository](https://github.com/GameBit/GameBit2) but are not part of
...
Currently only `Source10` is used, for the example `GameBit.conf` file.
...
in the GameBit git repository. At present only an SVG version of the GameBit
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the GameBit
...
needed when 0.12.1 is released, a proper fix is already in the GameBit git
master branch.
...
idea to build GameBit Core against a static version of that library compiled
...
The `GameBit.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `GameBit-server` package. At this point, most RPM based
...
The `GameBit-server` package should have SELinux support. How to properly do
...
GameBit-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/GameBit/). He has been
packaging GameBit for Fedora at least since 2012.
...
4. I renamed the `GameBit` package that contains the Qt GUI to `GameBit-core` as
that appears to be how the general population refers to it, in contrast to
`GameBit-xt` or `GameBit-classic`. I wanted to make sure the general population
